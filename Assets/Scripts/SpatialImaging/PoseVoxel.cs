...
namespace CloakingBox
{
    public class PoseVoxel
    {
        ...
        {
            // GetHashCode for Vector3 = this.x.GetHashCode() ^ this.y.GetHashCode() << 2 ^ this.z.GetHashCode() >> 2;

            int hash = Max.GetHashCode() ^ Min.GetHashCode() << 14;
            Debug.Log("Hash = " + hash);
            return hash;
        }
        ...
        {
            return "PoseVoxel[Min: {" + Min + "}; Max: {" + Max + "}]";
        }
